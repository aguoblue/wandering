# 20260426 my-app 会话列表后端未连接提示优化

1. **根因定位**：刷新时 `TravelChatPanel` 会重新请求 `/api/conversations`；当前本地 8787 AI 后端未运行，Vite 代理失败后前端落入通用“加载会话列表失败”文案，容易误以为是“删除全部会话”造成的空列表错误。
2. **前端提示优化**：`chatClient.listConversations` 增加网络/代理失败识别，后端未连接时提示在 `my-app` 目录运行 `npm run ai:server` 后刷新；真正空会话列表仍展示正常空态。
...
# 20260426 my-app 增加版本化用户画像

1. **画像存储与版本**：`my-app/server/ai_server.py` 新增 `user_profiles` 与 `user_profile_versions`，保存 Markdown 用户画像、当前版本、自动更新开关和每次更新来源。
2. **增量总结**：`messages` 增加画像总结版本标记，AI 更新画像时只读取未总结消息，生成新版画像后回写版本号，避免重复总结历史对话。
3. **对话注入画像**：会话流式回复会把当前用户画像加入 system prompt，作为长期偏好参考；若本次用户输入与画像冲突，仍优先遵循本次输入。
4. **主页画像入口**：主页顶部新增用户画像图标按钮，弹窗支持查看/编辑 Markdown、保存新版本、开启 AI 自动更新，以及手动触发 AI 总结更新。

# 20260426 my-app 工具优先服务接入用户画像

1. **接口对齐**：`my-app/server/ai_server_tools.py` 增加与主服务一致的 `/api/user-profile`、保存、自动更新开关和 AI 总结接口，前端画像弹窗可直接配合 `npm run ai:server:tools` 使用。
2. **Tool-first 注入画像**：工具优先对话流读取当前 Markdown 用户画像，并注入 Anthropic tool use 的 system prompt，生成计划、改计划和普通回复都能参考长期偏好。
3. **自动更新联动**：工具优先流式回复落库后会触发同一套自动画像更新逻辑，继续使用未总结消息与版本号避免重复消费。

# 20260426 my-app 增加路线质量提示词规则

1. **区域收敛**：计划生成与修改提示词新增“一天内尽量只在 1 个大区域或相邻区域”的约束，减少跨区域跳跃。
2. **单向推进**：新增“路线单方向推进、避免折返”的规则，降低来回绕路的行程。
3. **体验去重**：新增“同类型体验不要连续出现太多次”的规则，避免连续安排多次看海或相似打卡点。

# 20260426 my-app 活动详情增加用户感受备注

1. **活动备注存储**：`plansStore` 新增按 `planId + activityId` 保存的本地活动备注，不改动 AI 生成的 TravelPlan schema。
2. **详情页联动**：计划详情页进入时读取当前计划备注，用户输入后即时保存，后续重新打开计划仍可查看。
3. **展开区输入**：每个活动展开后新增“我的感受”备注输入框，方便记录体验、踩坑和下次安排想法。

# 20260426 my-app 新增用户足迹地图

1. **足迹来源**：计划详情页新增“标记已完成”，用户确认后将该计划中的活动地点写入本地足迹状态，并记录计划完成状态。
2. **足迹入口**：主页顶部在“用户画像”旁新增“用户足迹”按钮，可打开足迹地图弹窗查看已去过地点。
3. **地图点亮**：足迹地图使用金色编号点标记具体地点，并为每个地点绘制约 5km 半透明覆盖圆，表达去过区域范围。

# 20260426 my-app 主页计划卡片显示完成状态

1. **完成角标**：主页 `TravelPlanCard` 读取计划用户状态，已完成计划会在封面右上角显示“已完成”角标。
2. **状态一致**：角标复用计划详情页“标记已完成”写入的本地状态，和用户足迹来源保持一致。

# 20260426 my-app 计划与足迹迁移到后端数据库

1. **后端计划表**：`ai_server.py` 新增 `travel_plans`、`plan_user_states`、`activity_notes`、`visited_places` 表，并提供计划增删查、完成状态、活动备注和足迹 API。
2. **工具优先入口同步**：`ai_server_tools.py` 暴露同样的计划/足迹 API，使用 `npm run ai:server:tools` 时前端也能读写后端数据库。
3. **前端 API 化**：`plansStore` 改为 API 优先，主页、详情页、足迹弹窗和 AI 生成计划保存逻辑均改为读写后端；后端不可用时保留本地缓存兜底。
4. **本地计划迁移**：首次加载计划时会尝试把旧 localStorage 里的生成计划导入后端，避免已有计划在迁移后消失。

# 20260426 my-app 对话提示词注入用户足迹

1. **足迹摘要**：后端从 `visited_places` 读取全部已点亮地点，生成 Markdown 风格的用户足迹摘要，包含地点名、访问次数和最近访问日期。
2. **记忆注入**：`ai_server.py` 与 `ai_server_tools.py` 的对话请求都会同时注入用户画像和用户足迹，帮助 AI 避免重复安排已去过地点。
3. **边界规则**：提示词明确足迹只是事实，不代表偏好；除非用户要求重游或补完，否则默认避开重复地点。
